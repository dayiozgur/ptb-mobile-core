...
import '../microsoft/ms_onedrive_picker.dart';
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      const SizedBox(height: AppSpacing.md),
                      _ContactFilesCard(contactId: widget.contactId),
                      const SizedBox(height: AppSpacing.md),
...
/// Kişiye bağlı OneDrive/SharePoint dosyaları — web "Dosyalar" kartının mobil
/// karşılığı. `entity_file_links`'ten listeler; bağlıysa OneDrive picker ile
/// ekler, satırdan kaldırır. Dosya OneDrive'da kalır; yalnızca referans tutulur.
class _ContactFilesCard extends StatefulWidget {
  final String contactId;
  const _ContactFilesCard({required this.contactId});

  @override
  State<_ContactFilesCard> createState() => _ContactFilesCardState();
}

class _ContactFilesCardState extends State<_ContactFilesCard> {
  ...
  Future<void> _load() async {
    ...
    final files = await _links.list('contact', widget.contactId);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _attach() async {
    await showMsOneDrivePicker(context, entityType: 'contact', entityId: widget.contactId);
    await _load();
  }

...
/// Erişim Talebi (Waitlist) ekranı — PMS.
///
/// Invite-only: açık kayıt yoktur. Bu ekran `access_requests` tablosuna bir
/// erişim talebi yazar (platform_code='PMS', source='mobile', status varsayılan
/// 'pending'). Talep yönetici tarafından incelenir; onaylanırsa davet gönderilir.
...
class _RequestAccessScreenState extends State<RequestAccessScreen> {
  ...
  Future<void> _handleRequestAccess() async {
    ...
    try {
      ...
      if (mounted) {
        AppSnackbar.showError(context, message: _t('auth.request_access.error'));
      }
    } finally {
      if (mounted) {
            ...
            children: [
              ...
              AppTextField(
                ...
                validator: Validators.required(
                    _t('auth.request_access.full_name_required')),
